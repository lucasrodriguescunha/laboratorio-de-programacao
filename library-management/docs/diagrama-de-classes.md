...
├── app/
│   └── LibraryMenu.java                 ●  menu de console; único lugar que trata exceções
├── interfaces/
...
LibraryMenu ..> LibraryException : captura
```

Todas as exceções do domínio herdam de `LibraryException`. O menu faz **um**
`catch (LibraryException e)` e mostra `e.getMessage()` — sem precisar conhecer
cada caso individualmente. Isso é polimorfismo aplicado a erros.
...
## 5. Fluxo de "realizar empréstimo"

Onde cada exceção nasce:

```mermaid
flowchart LR
    A["LibraryMenu chama<br>create(matrícula, código, dias)"] --> B{"membro<br>existe?"}
...
## 7. Funcionalidades do enunciado → onde ficam
...
## 8. O que vem do projeto `library-system`
